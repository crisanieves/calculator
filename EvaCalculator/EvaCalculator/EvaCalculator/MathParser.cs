        ...
        public MathParser(bool loadPreDefinedFunctions = true, bool loadPreDefinedOperators = true, bool loadPreDefinedVariables = true, CultureInfo cultureInfo = null)
        {
            ...
            if (loadPreDefinedFunctions)
            {
                LocalFunctions = new Dictionary<string, Func<double[], double>>(26)
                {
                    ...
                    ["cos"] = inputs => Math.Round( Math.Cos(inputs[0]), 15, MidpointRounding.ToEven ),
                    ["cosh"] = inputs => Math.Cosh(inputs[0]),
                    ["acos"] = inputs => Math.Acos(inputs[0]),
                    ["arccos"] = inputs => Math.Acos(inputs[0]),

                    ["sin"] = inputs => Math.Round( Math.Sin(inputs[0]), 15, MidpointRounding.ToEven ),
                    ...
                    ["tan"] = inputs => 
                    {
                       if ( Math.Abs(inputs[0]) == (1.5707963267949) || Math.Abs(inputs[0]) == (4.71238898038469))
                       {    inputs[0] = 2 * Math.Pow(10, 308);

                            #region Cargar Idioma
                            if (SaveAndGet.Helpers.Settings.IdiomA == "Español" || SaveAndGet.Helpers.Settings.IdiomA == "Spanish")
                            {
                                MensajeTang = "El resultado no está definido o tiende a infinito para al menos un valor de x (±(π/2) o ±(1.5π)) en una función tangente.";
                            }
                            else if (SaveAndGet.Helpers.Settings.IdiomA == "Inglés" || SaveAndGet.Helpers.Settings.IdiomA == "English")
                            {
                                MensajeTang = "It is not defined or tends to infinite for at least a x value (±(π/2) o ±(1.5π)) in a tangent function.";
                            }
                            #endregion

                            Toast.MakeText(Android.App.Application.Context, MensajeTang, ToastLength.Long).Show();

                       } else {}

                            return Math.Round(Math.Tan(inputs[0]), 15, MidpointRounding.ToEven);
                    },
                    ...
                    #region DegreeFuntions() -->

                    ["ċos"] = inputs => Math.Round( Math.Cos( ((Math.PI) / 180) * inputs[0]), 15, MidpointRounding.ToEven ),
                    ["ċosh"] = inputs => Math.Cosh(inputs[0]),
                    ["arcċos"] = inputs => ((180 / Math.PI)) * Math.Acos(inputs[0]),
                    ["arcċosh"] = inputs => Math.Log(inputs[0] + Math.Sqrt(Math.Pow(inputs[0], 2) - 1)),
                    ["arccosh"] = inputs => Math.Log(inputs[0] + Math.Sqrt(Math.Pow(inputs[0], 2) - 1)),

                    ["siɳ"] = inputs => Math.Round( Math.Sin( ((Math.PI) / 180) * inputs[0]), 15, MidpointRounding.ToEven ),
                    ["siɳh"] = inputs => Math.Sinh(inputs[0]),
                    ["arcsiɳ"] = inputs => ((180 / Math.PI)) * Math.Asin(inputs[0]),
                    ["arcsiɳh"] = inputs => Math.Log(inputs[0] + Math.Sqrt(Math.Pow(inputs[0], 2) + 1)),
                    ["arcsinh"] = inputs => Math.Log(inputs[0] + Math.Sqrt(Math.Pow(inputs[0], 2) + 1)),

                    ["taɳ"] = inputs => 
                    {
                        if ( Math.Abs(inputs[0]) == 90 || Math.Abs(inputs[0]) == 270 )
                        {   inputs[0] = 2 * Math.Pow(10, 308);

                            #region Cargar Idioma
                            if (SaveAndGet.Helpers.Settings.IdiomA == "Español" || SaveAndGet.Helpers.Settings.IdiomA == "Spanish")
                            {
                                MensajeTang = "El resultado no está definido o tiende a infinito para al menos un valor de x (±90° o ±270°) en una función tangente.";
                            }
                            else if (SaveAndGet.Helpers.Settings.IdiomA == "Inglés" || SaveAndGet.Helpers.Settings.IdiomA == "English")
                            {
                                MensajeTang = "It is not defined or tends to infinite for at least a x value (±90° o ±270°) in a tangent function.";
                            }
                            #endregion
                            ...
                            return Math.Round(Math.Tan(((Math.PI) / 180) * inputs[0]), 15, MidpointRounding.ToEven);
                    },

                    ["taɳh"] = inputs => Math.Tanh(inputs[0]),
                    ["arctaŋ"] = inputs => ((180 / Math.PI)) * Math.Atan(inputs[0]),
                    ["arctaŋh"] = inputs => (Math.Log(((1 + inputs[0]) / (1 - inputs[0]))) * (0.5) ),
                    ["arctanh"] = inputs => (Math.Log(((1 + inputs[0]) / (1 - inputs[0]))) * (0.5) ),
